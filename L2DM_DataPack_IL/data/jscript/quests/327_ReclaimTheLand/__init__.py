 ...
 def onEvent (self,event,st) :
    ...
    if event == "30597-03.htm" :
      ...
      if n == 0 :
        ...
        st.addExpAndSp(int(n*152*Config.RATE_QUESTS_REWARD),0)
      ...
      if n == 0 :
        ...
        st.addExpAndSp(int(n*182*Config.RATE_QUESTS_REWARD),0)
        ObjectId=st.getPlayer().getObjectId()
        st.getPlayer().broadcastPacket(SocialAction(ObjectId,3))
        st.playSound("ItemSound.quest_itemget")
      ...
      for i in range(1852,1856) :
         ...
         if n :
           n1 = 1
           st.takeItems(i,n)
           st.addExpAndSp(int(n*EXP[i]*Config.RATE_QUESTS_REWARD),0)
           ObjectId=st.getPlayer().getObjectId()
           st.getPlayer().broadcastPacket(SocialAction(ObjectId,3))
           st.playSound("ItemSound.quest_itemget")
      if not n1 :
