	...
	protected final void writeImpl()
	{
		...
		for (L2ItemInstance item : _list)
		{
			if(item.getCount() >0 || item.getCount() == -1){
	            ...
	            if (item.getItemId() >= HardcodedItemTable.MERCENARY_POSTING_TICKET_1 && item.getItemId() <= HardcodedItemTable.MERCENARY_POSTING_TICKET_0)//Config.RATE_SIEGE_GUARDS_PRICE-//'
