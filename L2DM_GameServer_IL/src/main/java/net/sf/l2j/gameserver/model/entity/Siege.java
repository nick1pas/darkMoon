...
public class Siege
{
    ...
    {
        _defenderRespawnDelayPenalty += Config.SIEGE_CT_LOSS_PENALTY; // Add respawn penalty to defenders for each control tower lose
    }
    ...
    private boolean checkIfCanRegister(L2PcInstance player, int typeId)
    {
        ...
        {
        	player.sendMessage("Only clans with at least"+Config.SIEGE_CLAN_MIN_MEMBERCOUNT+" members or higher may register for a castle siege.");
        	return false;
        }
        //L2EMU_ADD
    ...
    private void spawnArtifact(int Id)
    {
        ...
        for (SiegeSpawn _sp: SiegeManager.getInstance().getArtefactSpawnList(Id))
        {
        	...
        	art.getStatus().setCurrentHpMp(art.getMaxHp(), art.getMaxMp());
