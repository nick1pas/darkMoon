...
public class L2Attackable extends L2NpcInstance
{
	...
	*/
    ...
    public boolean doDie(L2Character killer)
    {
        ...
        //L2EMU_ADD_START
        //checks if player is in jail mision
        if (killer instanceof L2PlayableInstance && ((L2PlayableInstance)killer).isInJailMission())
        {
        	...
        	if(!((L2PlayableInstance)killer).hasCompletedMission())
        	{
        		...
        		if(Rnd.get(100) <= Config.JAIL_POINT_CHANCE)
        		{
        			...
        			//informs player ammount of items he earned.
        			killer.sendMessage("You have earned "+Config.POINTS_PER_KILL+" jail point(s).");
        		}
        	}
        	else
        	{
        		if(Config.DEVELOPER)
        		_log.info("JailManager: Not needed to increase jail point(s), player "+((L2PlayableInstance)killer).getName()+" already completed points request."); 
        		
        		//infoms player that he already finisahed jail mission
        		((L2PlayableInstance)killer).sendMessage("Now you have all points, talk to jail manager.");
    ...
    private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
    {
        ...
        if (Config.DEEPBLUE_DROP_RULES)
        {
            if (levelModifier > 0)
            {
                ...
                deepBlueDrop = 3;
                if (drop.getItemId() == HardcodedItemTable.ADENA_ID) deepBlueDrop *= (int)Config.RATE_DROP_ADENA;
            }
        }
        ...
        // Applies Drop rates
        if (drop.getItemId() == HardcodedItemTable.ADENA_ID) dropChance *= Config.RATE_DROP_ADENA;
...
//         if (drop.getItemId() == HardcodedItemTable.ADENA_ID && dropChance > L2DropData.MAX_CHANCE) dropChance = L2DropData.MAX_CHANCE; // If item is adena, dont drop multiple time
        ...
        else if (itemCount == 0 && _log.isDebugEnabled()) _log.info("Roll produced 0 items to drop...");
    ...
    public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker)
    {
        ...
        for(L2DropCategory cat:npcTemplate.getDropData())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //L2EMU_ADD_BEGIN 
                	//Tuning drop for OFF, Theoretically drop 1-3 items categoty 1 or 2
            	    //There is a small chance drop 3-th item
            	    //Real chance poorly depends of rate drop
            	    //Author Visor123
                	if ((cat.getCategoryType() == 1)||(cat.getCategoryType() == 2)) 
                	{
                		...
                		if (Math.round(Rnd.get(3)) == 1) // 33% chance for add drop another item
                		{
                        	    ...
                        	    {
                        	    	if(item2.getItemId() == item.getItemId()) item2 = calculateCategorizedRewardItem(player, cat, levelModifier); 
                        	    	if (item2 != null)
                        	    		if(item2.getItemId() == item.getItemId()) item2 = null; //if 2-th duplicate, not drop this item
                        	    }
                                // Check if the autoLoot mode is active
                                if (item2 != null)
                                {
                                    if (_log.isDebugEnabled()) _log.info("Item2 id to drop: " + item2.getItemId() + " amount: " + item2.getCount());
                                    if (Config.AUTO_LOOT) player.doAutoLoot(this, item2); // Give this or these Item(s) to the L2PcInstance that has killed the L2Attackable
                                    else DropItem(player, item2); // drop the item on the ground
                                }
                		}	    
                	}
                	//L2EMU_ADD_END	
        ...
        if (rateHp <= 1 && String.valueOf(npcTemplate.getType()).contentEquals("L2Monster")) //only L2Monster with <= 1x HP can drop herbs
        {
            ...
            {                  
              ...
              //L2EMU_EDIT_START
                if (Config.AUTO_LOOT_HERBS || player.isAutoLootHerbs()) player.addItem("Loot", item.getItemId(), item.getCount(), this, true); 
                else DropItem(player, item);
                _spec = true;
            }
