...
import net.sf.l2j.gameserver.hardcodedtables.HardcodedNpcTable;
...
public class SevenSigns 
{
    ...
    public static final int ANCIENT_ADENA_ID = HardcodedItemTable.ANCIENT_ADENA_ID;
    ...
    public SevenSigns() 
    {
        ...
        if (isSealValidationPeriod())
            ...
            if (getCabalHighestScore() == CABAL_NULL)
                _log.info("SevenSigns: competition will end in a tie this week.");
            else
    ...
    {
    	HardcodedNpcTable hnt = new HardcodedNpcTable();
    	
        _merchantSpawn = AutoSpawnHandler.getInstance().getAutoSpawnInstance(hnt.MAMMON_MERCHANT_ID, false);
        _blacksmithSpawn = AutoSpawnHandler.getInstance().getAutoSpawnInstance(hnt.MAMMON_BLACKSMITH_ID, false);
        _marketeerSpawns = AutoSpawnHandler.getInstance().getAutoSpawnInstances(hnt.MAMMON_MARKETEER_ID);
        _spiritInSpawn = AutoSpawnHandler.getInstance().getAutoSpawnInstance(hnt.SPIRIT_IN_ID, false);
        _spiritOutSpawn = AutoSpawnHandler.getInstance().getAutoSpawnInstance(hnt.SPIRIT_OUT_ID, false);
        _lilithSpawn = AutoSpawnHandler.getInstance().getAutoSpawnInstance(hnt.LILITH, false);
        _anakimSpawn = AutoSpawnHandler.getInstance().getAutoSpawnInstance(hnt.ANAKIM, false);
        ...
        _oratorSpawns = AutoSpawnHandler.getInstance().getAutoSpawnInstances(hnt.ORATOR_NPC_ID_1);
        _preacherSpawns = AutoSpawnHandler.getInstance().getAutoSpawnInstances(hnt.PREACHER_NPC_ID_1);
        ...
        if (isSealValidationPeriod() || isCompResultsPeriod())
        {
            ...
            if (getSealOwner(SEAL_GNOSIS) == getCabalHighestScore() && getSealOwner(SEAL_GNOSIS) != CABAL_NULL)
            {
               ...
               _log.warn("SevenSigns: No spawn found for Orator of Revelations npc(s).");  
				...
				if (!AutoChatHandler.getInstance().getAutoChatInstance(hnt.PREACHER_NPC_ID_1, false).isActive() && !AutoChatHandler.getInstance().getAutoChatInstance(hnt.ORATOR_NPC_ID_1, false).isActive())
                    AutoChatHandler.getInstance().setAutoChatActive(hnt.PREACHER_NPC_ID_1,true);
                  }  
              else  
            ...
            else
            {
                ...
                AutoChatHandler.getInstance().setAutoChatActive(hnt.ORATOR_NPC_ID_1,false);
                AutoChatHandler.getInstance().setAutoChatActive(hnt.PREACHER_NPC_ID_1,false);
            }
