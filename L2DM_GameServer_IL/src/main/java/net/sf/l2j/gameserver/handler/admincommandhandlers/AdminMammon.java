...
public class AdminMammon implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance admin)
	{
		...
		if (command.startsWith("admin_mammon_find"))
		{
			...
			{
				admin.sendMessage("Usage: //mammon_find [teleportIndex] (where 1 = Blacksmith, 2 = Merchant)");
			}

			if (!_isSealValidation)
			{
				admin.sendMessage("The competition period is currently in effect.");
				return true;
			}
			if (blackSpawnInst!=null)
			{
				...
				admin.sendMessage("Blacksmith of Mammon isn't registered for spawn.");
			if (merchSpawnInst!=null)
			{
				...
				admin.sendMessage("Merchant of Mammon isn't registered for spawn.");
		}
		...
		else if (command.startsWith("admin_mammon_respawn"))
		{
			...
			if (merchSpawnInst!=null)
			{
				...
				admin.sendMessage("The Merchant of Mammon will respawn in "+(merchRespawn/60000)+" minute(s).");
			}
			else
				admin.sendMessage("Merchant of Mammon isn't registered for spawn.");
			if (blackSpawnInst!=null)
			{
				...
				admin.sendMessage("The Blacksmith of Mammon will respawn in "+(blackRespawn/60000)+" minute(s).");
			}
			else
				admin.sendMessage("Blacksmith of Mammon isn't registered for spawn.");
		}
		...
		else if (command.startsWith("admin_list_spawns"))
		{
			...
			{
				admin.sendPacket(SystemMessage.sendString("Command format is //list_spawns <npcId|npc_name> [tele_index]"));
			}

			SpawnTable.getInstance().findNPCInstances(admin, npcId, teleportIndex);
		}
		...
		else if (command.startsWith("admin_msg"))
		{
			...
			{
				admin.sendMessage("Command format: //msg <SYSTEM_MSG_ID>");
				return true;
			}
			admin.sendPacket(new SystemMessage(SystemMessageId.getSystemMessageId(msgId)));
		}
		return true;
	}
