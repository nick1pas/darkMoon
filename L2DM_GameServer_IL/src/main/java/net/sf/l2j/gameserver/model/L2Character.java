...
public abstract class L2Character extends L2Object
{
    ...
    * <B><U> Concept</U> :</B><BR><BR>
    ...
    protected void doAttack(L2Character target)
    {
        ...
        if (this instanceof L2PcInstance)
        {
            ...
            if (target instanceof L2PcInstance)
            {
                ...
                //L2EMU_ADD_START
                //prevents attacks on same team members in TvT
                if (!Config.TVT_ALLOW_TEAM_ATTACKING && target instanceof L2PcInstance)
                {
                	if(((L2PcInstance)this).isSameTeamSide(((L2PcInstance)this),((L2PcInstance)target)))
                	{
                		sendMessage("you cannot attack same team members!");
                		ActionFailed af = new ActionFailed();
                		sendPacket(af);
                		return;
                	}
                }
                //if a player is meditating and is being attacked, the effect is removed.
                cancelEffectMeditation(((L2PcInstance)target));
                //L2EMU_ADD_END
    ...
    public void doCast(L2Skill skill)
    {
        ...
        else
        {
            ...
            //L2EMU_ADD
            //if a player is meditating and is being attacked, the effect must be removed.
            cancelEffectMeditation(target);
            //L2EMU_ADD
        }            
       ...
       boolean forceBuff = skill.getSkillType() == SkillType.FORCE_BUFF && (target instanceof L2PcInstance);
        ...
        //L2EMU_ADD
        //adds a reuse delay to cp potions
        if(skill.isPotion() &&  skill.getSkillType() == SkillType.COMBATPOINTHEAL //add delay to cp potions
        		|| skill.getSkillType()== SkillType.MANAHEAL
        		|| skill.getSkillType()== SkillType.MANAHEAL_PERCENT)//add delay to mana heal potions
        {
        	if (skill.getId() >= 2287 && skill.getId() <= 2289)
        	{
        		reuseDelay = Config.ELIXIRS_DELAY;
        	}
        	else
        	{
            	reuseDelay = Config.POTIONS_DELAY;
        	}
        	if (reuseDelay > 0)
	...
	*/
	...
	public boolean doDie(L2Character killer)
	{
		...
		/**
		 * checks for enabled faster target revalidation: 
		 * revalidates targets from dead mob, removing any targets from that corpse
		 * some seconds before decay, if config enabled.
		 */
		if(this instanceof L2PlayableInstance && Config.FASTER_TARGET_REVALIDATION)
		{
			//wait some seconds only before remove target instead of remove only after corpse decay.
			ThreadService.processSleep(6);

			//"sleep" this thread and let others continue their work
			Thread.yield(); 

			//finally we remove killer target
			killer.setTarget(null); 

			//now lets "wake-up" the thread again 
			Thread.activeCount();  
		}
		//checks if jail spawn system reduces player jail points on death
		if(this instanceof L2PlayableInstance && ((L2PlayableInstance)this).isInJailMission() && Config.REDUCE_JAIL_POINTS_ON_DEATH)
		{
			...
			sendMessage("You lost "+Config.POINTS_LOST_PER_DEATH+" jail point(s) due to death.");
			if(Config.DEVELOPER)_log.info("JailManager: Reduced "+getName()+" jail point(s) due to death.");
		}
		...
		else
		{
			...
			else
			{
				...
				//  checks for enabled buff keeping config.(not working properly)
				switch(Config.KEEP_BUFFS_ON_DEATH)
				{
				case DONATOR:
				{
					//checks donator enum enabled, if true donators will not loose their buffs.
					if(this instanceof L2PlayableInstance && ((L2PlayableInstance)this).isDonator())
						return false;
				...
				case EVERYBODY:
				{
					//nothing to do, no one will loose theiur buffs.
					if(this instanceof L2PlayableInstance)
						return false;
				...
				case NONE:
				{
					//everybody just stop all effects in progress of any instance without distictions.
					stopAllEffects();
					break;
				}
				case GM:
				{
					//returns if is a GM. and gm enum is enabled.
					if(this instanceof L2PlayableInstance && ((L2PlayableInstance)this).isGM())
						return false;
    ...
    public void doRevive()
    {
        if (!isTeleporting())
        {
            ...
            //just a lil delay between revive.
   ...
   public final void addEffect(L2Effect newEffect)
   {
       ...
       synchronized(_effects) 
       {
    	   ...
    	   if (getBuffImmunity() && newEffect.getEffectType() != L2Effect.EffectType.BUFFIMMUNITY)
    	   {
    		   ...
    		   if (typeSkill == L2Skill.SkillType.BUFF || typeSkill == L2Skill.SkillType.DEBUFF) return;
    	   }
    	   //L2EMU_ADD
...
// private int _floatingRunSpeed;
	...
	*/
    ...
    private boolean _meditating;	             //True if the L2Character is using the a meditation skill
    ...
    public final void setBuffImmunity(boolean value) 
    {
	 ...
	 /**
     * removes meditation effect of target
     * 
     */
    public void cancelEffectMeditation(L2Character target)
    {
    	if (target.isMeditating())
    	{
    		...
    		for (L2Effect e : effects)
    		{
    			if (e.getEffectType() == L2Effect.EffectType.MEDITATION)
    				e.exit();

    			// tell about it to player
    			SystemMessage sm = new SystemMessage(SystemMessageId.EFFECT_S1_DISAPPEARED);
    			sm.addString(e.getSkill().getName());
    			target.sendPacket(sm);
    		}
    	}
    }
    public void setMeditating(boolean val)
    {
    	_meditating = val;
    }
