...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance admin)
	{
		...
		if (command.startsWith("admin_showShop "))
		{
			...
			if (args.length > 1)
				showShop(admin, Integer.parseInt(command.split(" ")[1]));
		}
		else if(command.startsWith("admin_showShopList "))
		{
			String[] args = command.split(" ");
			if (args.length > 2)
				showShopList(admin, Integer.parseInt(command.split(" ")[1]), Integer.parseInt(command.split(" ")[2]));
		}
		else if(command.startsWith("admin_edit_npc "))
		{
			try
			{
				...
				Show_Npc_Property(admin, npc);
			}
			catch (Exception e)
			{
				admin.sendMessage("Wrong usage: //edit_npc <npcId>");
			}
		}
		else if(command.startsWith("admin_load_npc"))
		{
			...
			{
				admin.sendMessage("Usage: //load_npc <id>");
			}
			if(id > 0)
			{
				if(NpcTable.getInstance().reloadNpc(id))
					admin.sendMessage("Loaded NPC "+id);
				else
					admin.sendMessage("Error while loading NPC "+id);
			}
		}
		else if(command.startsWith("admin_show_droplist "))
		{
			...
			if(npcId > 0)
			{
				showNpcDropList(admin, npcId);
			}
			else
			{
				admin.sendMessage("Usage: //show_droplist <npc_id>");
			}
		}
		else if (!Config.ALT_PRIVILEGES_ADMIN && !(checkLevel(admin.getAccessLevel()) && admin.isGM()))
			return false;
		else if(command.startsWith("admin_addShopItem "))
		{
			String[] args = command.split(" ");
			if (args.length > 1)
				addShopItem(admin, args);
		}
		else if(command.startsWith("admin_delShopItem "))
		{
			String[] args = command.split(" ");
			if (args.length > 2)
				delShopItem(admin, args);
		}
		else if(command.startsWith("admin_editShopItem "))
		{
			String[] args = command.split(" ");
			if (args.length > 2)
				editShopItem(admin, args);
		}
		else if(command.startsWith("admin_save_npc "))
		{
			try
			{
				save_npc_property(admin, command);
			}
		...
		else if(command.startsWith("admin_show_skilllist_npc "))	// L2J_JP ADD
		{
			...
			try
			{
				...
				if(npcId > 0)
				{
					showNpcSkillList(admin, npcId, page);
				}
				else
					admin.sendMessage("Usage: //show_skilllist_npc <npc_id> <page>");
			}
			catch (Exception e)
			{
				admin.sendMessage("Usage: //show_skilllist_npc <npc_id> <page>");
			...
			try
			{
				...
				if (st.countTokens() == 2)
				{
					try
					{
						...
						showNpcSkillEdit(admin, npcId, skillId);
					}
					catch(Exception e)
					{}
				}
				else if (st.countTokens() == 3)
				{
					try
					{
						...
						updateNpcSkillData(admin, npcId, skillId, level);
					}
					catch(Exception e)
					{
				...
				{
					admin.sendMessage("Usage: //edit_skill_npc <npc_id> <item_id> [<level>]");
				}
			}
			catch (StringIndexOutOfBoundsException e)
			{
			...
			try
			{
				...
				else if (st.countTokens() == 3)
				{
					try
					{
						...
						addNpcSkillData(admin, npcId, skillId, level);
					}
					catch(Exception e)
					{
				...
				{
					admin.sendMessage("Usage: //add_skill_npc <npc_id> [<level>]");
				}
			}
			catch (StringIndexOutOfBoundsException e)
			{
		...
		else if(command.startsWith("admin_del_skill_npc "))	// [L2J_JP ADD]
		{
			...
			if(npcId > 0)
			{
				deleteNpcSkillData(admin, npcId, skillId);
			}
			else
			{
				admin.sendMessage("Usage: //del_skill_npc <npc_id> <skill_id>");
			}
		}		
			...
			try
			{
				...
				{			
					try
					{
						...
						showEditDropData(admin, npcId, itemId, category);
					}
					catch(Exception e)
					{}
				}
				else if(st.countTokens() == 6)
				{
					try
					{
						...
						updateDropData(admin, npcId, itemId, min, max, category, chance);
					}
					catch(Exception e)
					{
					...
					admin.sendMessage("Usage: //edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
			}
			catch (StringIndexOutOfBoundsException e)
			{
				admin.sendMessage("Usage: //edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
			...
			try
			{
				...
				else if( st.countTokens() == 6)
				{
					try
					{
						...
						addDropData(admin, npcId, itemId, min, max, category, chance);
					}
					catch(Exception e)
					{
					...
					admin.sendMessage("Usage: //add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
			}
			catch (StringIndexOutOfBoundsException e)
			{
				admin.sendMessage("Usage: //add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
		...
		else if(command.startsWith("admin_del_drop "))
		{
			...
			if(npcId > 0)
				deleteDropData(admin, npcId, itemId, category);
			else
				admin.sendMessage("Usage: //del_drop <npc_id> <item_id> <category>");
		}
	...
	private void Show_Npc_Property(L2PcInstance admin, L2NpcTemplate npc)
	{
		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		//L2EMU_EDIT
		String content = HtmCache.getInstance().getHtm("data/html/admin/menus/submenus/editnpc_menu.htm");
		//L2EMU_EDIT
		if (content != null)
		{
			...
			//L2EMU_EDIT
			adminReply.setHtml("<html><body>File not found: data/html/admin/menus/editnpc_menu.htm</body></html>");
		//L2EMU_EDIT
		admin.sendPacket(adminReply);		
	}
	
	private void save_npc_property(L2PcInstance admin, String command)
	{
		String[] commandSplit = command.split(" ");
		...
		Show_Npc_Property(admin, NpcTable.getInstance().getTemplate(npcId));
	}
