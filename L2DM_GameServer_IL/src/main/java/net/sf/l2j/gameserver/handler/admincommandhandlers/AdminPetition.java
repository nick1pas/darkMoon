...
public class AdminPetition implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance admin)
	{
		...
		else if (command.startsWith("admin_accept_petition"))
		{
			...
			{
				admin.sendPacket( new SystemMessage(SystemMessageId.ONLY_ONE_ACTIVE_PETITION_AT_TIME));
				return true;
			}
			
			if (PetitionManager.getInstance().isPetitionInProcess(petitionId))
			{
				admin.sendPacket( new SystemMessage(SystemMessageId.PETITION_UNDER_PROCESS));
				return true;
			}
			
			if (!PetitionManager.getInstance().acceptPetition(admin, petitionId))
				admin.sendPacket( new SystemMessage(SystemMessageId.NOT_UNDER_PETITION_CONSULTATION));
		}
		else if (command.startsWith("admin_reject_petition"))
		{
			if (!PetitionManager.getInstance().rejectPetition(admin, petitionId))
				admin.sendPacket( new SystemMessage(SystemMessageId.FAILED_CANCEL_PETITION_TRY_LATER));
		}
