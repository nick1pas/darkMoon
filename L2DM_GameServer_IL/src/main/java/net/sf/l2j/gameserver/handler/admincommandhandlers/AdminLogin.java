...
public class AdminLogin implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance admin)
	{
		if (!Config.ALT_PRIVILEGES_ADMIN)
			if(admin.getAccessLevel() < REQUIRED_LEVEL)
				return false;
		
		if(command.equals("admin_server_gm_only"))
		{
			gmOnly();
			admin.sendMessage("Server is now GM only");
			showMainPage(admin);
		}
		else if(command.equals("admin_server_all"))
		{
			allowToAll();
			admin.sendMessage("Server is not GM only anymore");
			showMainPage(admin);
		}
		else if(command.startsWith("admin_server_max_player"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				try
				{
					...
					admin.sendMessage("maxPlayer set to "+new Integer(number).intValue());
					showMainPage(admin);
				}
				catch(NumberFormatException e)
				{
					admin.sendMessage("Max players must be a number.");
				}
			}
			else
			{
				admin.sendMessage("Format is server_max_player <max>");
			}
		}
		else if(command.startsWith("admin_server_list_clock"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				if(mode.equals("on"))
				{
					...
					admin.sendMessage("A clock will now be displayed next to the server name");
				...
				else if(mode.equals("off"))
				{
					...
					admin.sendMessage("The clock will not be displayed");
					showMainPage(admin);
				}
				else
				{
					admin.sendMessage("Format is server_list_clock <on/off>");
				}
			}
			else
			{
	...
	private void showMainPage(L2PcInstance admin)
	{
		...
		//L2EMU_EDIT
		html.setFile("data/html/admin/menus/menus/login_menu.htm");
		//L2EMU_EDIT
