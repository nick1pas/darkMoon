...
public class L2CastleWarehouseInstance extends L2FolkInstance
{
	...
	{
		player.sendPacket( new ActionFailed() );
		String filename = HtmlPathService.CASTLE_WAREHOUSE_HTML_PATH+"castlewarehouse-no.htm";
		
		int condition = validateCondition(player);
		if (condition > COND_ALL_FALSE) {
			if (condition == COND_BUSY_BECAUSE_OF_SIEGE)
				filename = HtmlPathService.CASTLE_WAREHOUSE_HTML_PATH+"castlewarehouse-busy.htm"; // Busy because of siege
			else if (condition == COND_OWNER) {                                  // Clan owns castle
				if (val == 0) 
					filename = HtmlPathService.CASTLE_WAREHOUSE_HTML_PATH+"castlewarehouse.htm";
				else
					filename = HtmlPathService.CASTLE_WAREHOUSE_HTML_PATH+"castlewarehouse-" + val + ".htm";
			}
		}
