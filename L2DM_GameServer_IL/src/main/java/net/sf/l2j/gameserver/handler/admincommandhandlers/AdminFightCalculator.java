...
public class AdminFightCalculator implements IAdminCommandHandler {
	...
	public boolean useAdminCommand(String command, L2PcInstance admin)
    {
        try
        {
    		...
    		if (command.startsWith("admin_fight_calculator_show"))
    			handleShow(command.substring("admin_fight_calculator_show".length()), admin);
    		else if (command.startsWith("admin_fcs"))
    			handleShow(command.substring("admin_fcs".length()), admin);
    		else if (command.startsWith("admin_fight_calculator"))
    			handleStart(command.substring("admin_fight_calculator".length()), admin);
        }
	...
	private void handleStart(String params, L2PcInstance admin)
	{
	...
	private void handleShow(String params, L2PcInstance admin) {
		...
		{
			npc1 = admin;
			npc2 = (L2Character)admin.getTarget();
			if(npc2 == null)
			{
