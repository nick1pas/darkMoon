...
public class AdminMonsterRace implements IAdminCommandHandler
{
    ...
    private void handleSendPacket(L2PcInstance admin)
    {
        /*
        ...
        if (state == -1)
        {
            ...
            admin.sendPacket(spk);
            admin.broadcastPacket(spk);
        }
        else if (state == 0)
        {
            state++;
            ...
            admin.sendPacket(SRace);
            admin.broadcastPacket(SRace);
            PlaySound SRace2 = new PlaySound(0, "ItemSound2.race_start", 1, 121209259, 12125, 182487, -3559);
            admin.sendPacket(SRace2);
            admin.broadcastPacket(SRace2);
            MonRaceInfo spk = new MonRaceInfo(codes[state][0], codes[state][1], race.getMonsters(), race.getSpeeds());
            admin.sendPacket(spk);
            admin.broadcastPacket(spk);

            ThreadPoolManager.getInstance().scheduleGeneral(new RunRace(codes, admin), 5000);
        }
    ...
    class RunRace implements Runnable
    {
        ...
        private int[][] codes;
        private L2PcInstance admin;

        public RunRace(int[][] pCodes, L2PcInstance padmin)
        {
            this.codes = pCodes;
            this.admin = padmin;
        }
        ...
        public void run()
        {
            ...
            admin.sendPacket(spk);
            admin.broadcastPacket(spk);
            ThreadPoolManager.getInstance().scheduleGeneral(new RunEnd(admin), 30000);
        }
    }
    ...
    class RunEnd implements Runnable
    {
        ...
        public RunEnd(L2PcInstance padmin)
        {
            this.admin = padmin;
        }
        ...
        public void run()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                admin.sendPacket(obj);
                admin.broadcastPacket(obj);
            }
            state = -1;
        }
    }
