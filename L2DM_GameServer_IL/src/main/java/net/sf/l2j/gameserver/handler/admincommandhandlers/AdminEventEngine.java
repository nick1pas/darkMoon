...
public class AdminEventEngine implements IAdminCommandHandler {
 ...
 private final static Log _log = LogFactory.getLog(AdminEventEngine.class);
	...
	public boolean useAdminCommand(String command, L2PcInstance admin) {
		...
		if (command.equals("admin_event")) showMainPage(admin);

        else if (command.equals("admin_event_new"))
		{
            showNewEventPage(admin);
		}
        ...
        else if (command.startsWith("admin_event_set")) 
        {
            ...
            showEventParameters(admin, 2);
        
        }
        else if (command.startsWith("admin_event_change_teams_number"))
        {
            showEventParameters(admin, Integer.parseInt(command.substring(32)));
                }
        else if (command.startsWith("admin_event_panel"))
        {
             showEventControl(admin);
         }
        ...
        else if (command.startsWith("admin_event_control_teleport"))
        {
            ...
            while(st.hasMoreElements()){
                teleportTeam(admin, Integer.parseInt(st.nextToken()));
            }
            showEventControl(admin);
        }
        ...
        else if (command.startsWith("admin_event_control_kill"))
        {
            ...
            while(st.hasMoreElements()){
                killTeam(admin, Integer.parseInt(st.nextToken()));
            }
            showEventControl(admin);
        }
        ...
        else if (command.startsWith("admin_event_control_prize"))
        {
            ...
            String id = st0.nextToken();
            while(st.hasMoreElements()){
                regardTeam(admin, Integer.parseInt(st.nextToken()),Integer.parseInt(n), Integer.parseInt(id), type);
            }
            showEventControl(admin);
        }
        ...
        else if (command.startsWith("admin_event_announce"))
        {
            ...
            muestraNpcConInfoAPlayers(admin, L2Event.id);
	...
	String showStoredEvents(){
        ...
        for (String element : files) {
            ...
            File file = new File("data/events/" + element);
    ...
    public void showNewEventPage(L2PcInstance admin)
    {
        NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
    ...
    public void showEventParameters(L2PcInstance admin, int teamnumbers)
    {
        NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
    ...
    void muestraNpcConInfoAPlayers(L2PcInstance admin, int id){
        L2Event.npcs.clear();
        ...
        L2Event.announceAllPlayers(admin.getName() + " wants to make an event !!! (you'll find a npc with the details around)");
    }
   void showEventControl(L2PcInstance admin){
   ...
   void teleportTeam(L2PcInstance admin, int team){
       ...
       while(it.hasNext()){
           ...
           pc.teleToLocation(admin.getX(), admin.getY(), admin.getZ());}catch(Exception e){}
       }
   ...
   void killTeam(L2PcInstance admin, int team){
       LinkedList linked = L2Event.players.get(team);
       Iterator it = linked.iterator();
       while(it.hasNext()){
           try{L2PcInstance target = L2World.getInstance().getPlayer(it.next().toString());
           target.reduceCurrentHp(target.getMaxHp() + target.getMaxCp() + 1, admin);}catch(Exception e){}
       }
   ...
   void resTeam(int team){
       ...
       while(it.hasNext()){
           ...
           character.getStatus().setCurrentCp(character.getMaxCp());
    ...
    private void createItem(L2PcInstance admin, L2PcInstance player, int id, int num)
    {
       player.getInventory().addItem("Event", id, num, player, admin);
    ...
    void regardTeam(L2PcInstance admin, int team, int n, int id, String type)
    {
        LinkedList linked = L2Event.players.get(team);
        int temp = n;
        Iterator it = linked.iterator();
        while(it.hasNext())
        {
            ...
            createItem(admin, target, id, temp);}catch(Exception e){}
        }
    }
