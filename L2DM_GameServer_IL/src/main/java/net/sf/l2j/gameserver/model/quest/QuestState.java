...
// END STUFF THAT WILL PROBABLY BE CHANGED
	...
	public void takeItems(int itemId, int count) 
    {
        ...
        //L2EMU_ADD
        if(itemId == 9141){
        	getPlayer().removeSkill(SkillTable.getInstance().getInfo(3260, 1));
        	getPlayer().removeSkill(SkillTable.getInstance().getInfo(3262, 1));
        }
        //L2EMU_ADD
	}
			...
			for (List<L2DropData> lst : getDrops().values()) {
		    	for (L2DropData d : lst) {
					...
					L2ItemInstance item = getPlayer().getInventory().getItemByItemId(itemId);
				    	...
				    	if (item == null || itemId == HardcodedItemTable.ADENA_ID)
						continue;
