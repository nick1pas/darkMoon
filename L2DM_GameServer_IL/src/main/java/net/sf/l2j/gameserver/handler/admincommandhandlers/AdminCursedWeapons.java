    ...
    public boolean useAdminCommand(String command, L2PcInstance admin)
    {
        ...
        if (command.startsWith("admin_cw_info"))
        {
			...
			else
			{
				...
				//L2EMU_EDIT
				adminReply.setFile("data/html/admin/info/cwinfo.htm");
				//L2EMU_EDIT
				for (CursedWeapon cw : cwm.getCursedWeapons())
				{
					itemId=cw.getItemId();
		...
		else
		{
			...
			try
			{
				...
				if (cw==null)
				{
					admin.sendMessage("Unknown cursed weapon ID.");
					return false;
				}
			}
			catch (Exception e)
			{
				admin.sendMessage("Usage: //cw_remove|//cw_goto|//cw_add <itemid|name>");
			}
			...
			else if (command.startsWith("admin_cw_add"))
			{
				if (cw==null)
				{
					admin.sendMessage("Usage: //cw_add <itemid|name>");
					return false;
				}
				else if (cw.isActive())
					admin.sendMessage("This cursed weapon is already active.");
				else
				{
