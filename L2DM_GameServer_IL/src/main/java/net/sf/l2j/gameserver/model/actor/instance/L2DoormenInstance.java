...
public class L2DoormenInstance extends L2FolkInstance
{
	...
	*/
    ...
    {
        player.sendPacket(new ActionFailed());
        String filename = HtmlPathService.DOORMEN_HTML_PATH + getTemplate().getNpcId() + "-no.htm";

        int condition = validateCondition(player);
        if (condition == COND_BUSY_BECAUSE_OF_SIEGE) filename = HtmlPathService.DOORMEN_HTML_PATH
            + getTemplate().getNpcId() + "-busy.htm"; // Busy because of siege
        else if (condition == COND_CASTLE_OWNER) // Clan owns castle
            filename = HtmlPathService.DOORMEN_HTML_PATH + getTemplate().getNpcId() + ".htm"; // Owner message window
