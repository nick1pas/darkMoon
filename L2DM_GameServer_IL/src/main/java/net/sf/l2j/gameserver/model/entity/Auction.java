...
public class Auction
{
	...
	private int _id								= 0;
	private int _adenaId						= HardcodedItemTable.ADENA_ID;
	...
	public void setBid(L2PcInstance bidder, int bid)
	{
	    ...
	    {
			if(takeItem(bidder, HardcodedItemTable.ADENA_ID, requiredAdena))
			{
    ...
    private void removeBids()
    {
        ...
        for (Bidder b : _bidders.values())
        {
          if (ClanTable.getInstance().getClanByName(b.getClanName()).getHasHideout() == 0)
        	  returnItem(b.getClanName(), HardcodedItemTable.ADENA_ID, 9*b.getBid()/10, false); // 10 % tax
          else
          {
    ...
    public void endAuction()
    {
    	if(ClanHallManager.getInstance() != null && ClanHallManager.getInstance().loaded())
		{
	        ...
	        if (_sellerId > 0)
	        {
	            returnItem(_sellerClanName, HardcodedItemTable.ADENA_ID, _highestBidderMaxBid, true);
	            returnItem(_sellerClanName, HardcodedItemTable.ADENA_ID, ClanHallManager.getInstance().getClanHall(_itemId).getLease(), false);
	        }
		    deleteAuctionFromDB();
    ...
    public void cancelBid(int bidder)
    {
        ...
        returnItem(_bidders.get(bidder).getClanName(), HardcodedItemTable.ADENA_ID, _bidders.get(bidder).getBid(), true);
