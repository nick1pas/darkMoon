...
public class L2CastleChamberlainInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (condition == COND_OWNER)
		{
			...
			else if (actualCommand.equalsIgnoreCase("receive_report"))
			{
				...
				html.setFile(HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain-report.htm");
			...
			{
				String filename = HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain-vault.htm";
				int amount = 0;
				...
				else if (val.equalsIgnoreCase("withdraw"))
				{
					...
					if (amount > 0)
					{
						...
						{
							filename = HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain-vault-no.htm";
						}
						else
						{
			...
			else if(actualCommand.equalsIgnoreCase("manor"))
			{
				...
				{
					filename = HtmlPathService.HTML_PATH+"npcdefault.htm";
				}
				else
				{
					...
					{
						case 0:
							filename = HtmlPathService.CASTLE_CHAMBERLAIN_MANOR_HTML_PATH+"manor.htm";
							break;
						case 4:
							filename = HtmlPathService.CASTLE_CHAMBERLAIN_MANOR_HTML_PATH+"manor_help00"+st.nextToken()+".htm";
							break;
						default:
							filename = HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain-no.htm";
							break;
					}
				}
			...
			else if(actualCommand.equalsIgnoreCase("operate_door")) // door control
			{
				...
				NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH + getTemplate().getNpcId() + "-d.htm");
				html.replace("%objectId%", String.valueOf(getObjectId()));
				html.replace("%npcname%", getName());
				player.sendPacket(html);
				return;
	...
	{
		player.sendPacket( new ActionFailed() );
		String filename = HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain-no.htm";
		
		int condition = validateCondition(player);
		if (condition > COND_ALL_FALSE)
		{
			if (condition == COND_BUSY_BECAUSE_OF_SIEGE)
				filename = HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain-busy.htm";	// Busy because of siege
			else if (condition == COND_OWNER)								// Clan owns castle
				filename = HtmlPathService.CASTLE_CHAMBERLAIN_HTML_PATH+"chamberlain.htm";			// Owner message window
		}
